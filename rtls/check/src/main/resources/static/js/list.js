	...
	$.ajax({
		...
		success:function(res){
        	...
        	for(var i = 0;i < list.length;i++){
				...
				if(curType == 2){
					html += '<td><span class="tabOper" onclick="download(\'' + list[i].tagcheckid + '\')">下载原始数据</span>';
				}else{
					html += '<td><span class="tabOper" onclick="download(\'' + list[i].tagcheckid + '\')">下载测试结果</span>';
				}
				html += '<span class="tabOper deleOpa" onclick="deleteItem(\'' + list[i].tagcheckid + '\')">删除</span></td></tr>';
