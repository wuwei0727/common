...
;(function($){
	$.fn.jcDate = function(options) {
		...
		return this.each(function() {
			...
			$dayFrame.undelegate("li.jcDateColor","dblclick").delegate("li.jcDateColor","click",function(e){
				submitDate(e);
			});
			...
			$submit.die().live("click",function(e){
				submitDate(e);
			});
			function submitDate(e){
				...
				var fillTarget = $("input.dateVisited");
				if(fillTarget.attr('id') == 'startTime'){
					dateInfo += ' 00:00:00';
				}else if(fillTarget.attr('id') == 'endTime'){
					dateInfo += ' 23:59:59';
				}
				fillTarget.val(dateInfo);
				closeDate();
			};
			$dateInput.addClass(options.IcoClass)
					  ...
					  .live(options.Event,function(e){
					  	window.seleTime && (window.seleTime = 'show');
			...
			$jcDate.die().live("click",function(e){
                //取消冒泡
			...
			function closeDate(){
				...
				window.seleTime && (window.seleTime = 'hide');
				return false;
			};
		});
	};
