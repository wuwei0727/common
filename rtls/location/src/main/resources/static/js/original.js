...
function search(num){
	...
	where.data_type=$('[name="data_type"]').val();
	...
	if (start) {
		...
		let stime = timeArr[timeArr.length - 1];
		let stimeArr = stime.split(':');
		newStart = start.split(' ')[0] + ' ' + str0(stimeArr[0]) + ':' + str0(stimeArr[1]) + ':' + str0(stimeArr[2]);
	}
	if (end) {
		let timeArr = end.split(' ');
		let etime = timeArr[timeArr.length - 1];
		let etimeArr = etime.split(':');
		newEnd = end.split(' ')[0] + ' ' + str0(etimeArr[0]) + ':' + str0(etimeArr[1]) + ':' + str0(etimeArr[2]);
	}

	where.start = newStart;
	where.end = newEnd;
	if (num == 1) {
		where.url = url + "kk/infraredDataAnalyze";
		// where.url=url+"kk/inFrared";
        where.pageIndex = 1;
        init();
    }else if(num==2){
		//where.url=url+"kk/exportFrared";
    where.url=url+"kk/exportFrared2";
		...
		where.url=url+"kk/exportFraredCount2";
        where.pageIndex = pageIndex;
		var formUrl = where.url;
		var form = $("<form></form>").attr("action", formUrl).attr("method", "post");
		var html = '';
