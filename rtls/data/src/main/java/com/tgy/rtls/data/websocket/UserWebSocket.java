 ...
 * @Author: wuwei
 * @CreateTime: 2023-07-04 09:22
 * @Description: TODO
 * @Version: 1.0
 */
@Component
@ServerEndpoint("/websocket/userWebSocket/{uid}")
@Slf4j
public class UserWebSocket {
    ...
    @PostConstruct
    public void init(){
        UserWebSocket userWebSocket = this;
    }
...
//            session_uid.remove(sessionId);
        ...
        // log.error("User-sessionId："+session.getId());
        // log.error("User-session_uid："+ session_uid.entrySet());
        // log.error("User-有新客户端连接了" + clients.size());
    }
    ...
    private void sendInitialData(Object message, String uid){
        if(!NullUtils.isEmpty(message) || !NullUtils.isEmpty(message)) {
            H5WebSocket h5WebSocket = SpringContextHolder.getBean(H5WebSocket.class);
            h5WebSocket.sendAll(message,uid);
