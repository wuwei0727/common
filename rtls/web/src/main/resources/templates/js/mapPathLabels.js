...
function init(){
    $.ajax({
        url: url + 'mapPathLabel/getMapPathLabel',
        data:where,
        type:'post',
        beforeSend:function(){
            loading();
...
function entDele(){
    $.ajax({
        url:url + 'mapPathLabel/delMapPathLabel/' + ids,
        beforeSend:function(){
            loading();
        },
        complete:function(){
...
function detail(id){
    ...
    $('#editFrame').attr('src', './mapPathLabelsDetails.html?id=' + id) 
    $('#editFrame').show()
}

function addMapPathLabels() {
    $('#addFrame').attr('style', 'z-index: 99') 
    $('#addFrame').attr('src', './mapPathLabelsDetails.html') 
    $('#addFrame').show()
}
