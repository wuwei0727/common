...
function entDele() {
  $.ajax({
    url: url + 'carPlate/delCarPlateById/' + ids,
    type: 'delete',
    beforeSend: function () {
      loading();
    },
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './licensePlateBindDetail.html?id=' + id)
  $('#editFrame').show()
}
...
function addCompany() {
  ...
  $('#addFrame').attr('src', './licensePlateBindDetail.html')
  $('#addFrame').show()
}
