...
function openMap2(para, callback) {
    ...
    fMap.on('loaded', function () {
        ...
        if (para.wantToDo === "drawArea") {
            ...
            callback && callback();
        } else if (para.wantToDo === "all") {
            initMapEditor(para.polygonColor);
            if (para.initPoints) {
                mapArr = [];
                initMapPoints(para.initPoints, para.polygonColor);
            }
            if (para.initCameraArea) {
                cArea = para.cArea;
                let data = JSON.parse(para.initCameraArea);
                initMapPoints([data], '#009944');
            }
        ...
        if (!para.noView) {
            ...
            let bound = fMap.getFloor(+floor).bound;;
    ...
    fMap.on('click', function (event) {
        ...
        if (para.wantToDo === 'drawArea') {
            ...
            if (CLICKTYPE == 'selectPoint') {
                aaSelectPoint(event)
            };
            if (CLICKTYPE == 'drawArea') {
                aaDrawArea(event)
            };
        }
    });
...
function aaDrawArea(event) {
    ...
    if (marker) {
        if (mapdraw.DrawTool === 'del') {
            ...
            if (marker.id === cameraAreaId) {
                initCameraArea([], 0, '');
                marker.remove();
                return false;
            } else if (marker.jn.uuid === cameraAreaId) {
                initCameraArea([], 0, '');
                marker.remove();
                return false;
            }
...
function initMapEditor(polygonColor, flag) {
    ...
    mapdraw.on('drawend', function (e) {
        ...
        if (cameraAreaType === 'vpolygon') {
            ...
            initCameraArea(e.info.points, fMap.getLevel(), e.info.id);
            return false;
        }
    ...
    mapdraw.on('editend', function (e) {
        ...
        console.log('cameraAreaId', cameraAreaId);
        console.log('e', e.info.id);

        if (cameraAreaId === e.info.id) {
            initCameraArea(e.info.points, fMap.getLevel(), e.info.id);
            return false;
        }
        if (first) {
...
function drawOpetion(type) {
    ...
    cameraAreaType = '';
    if (type === 'del2') {
        mapdraw.DrawTool = 'del';
    } else if (type === 'vpolygon') {
        cameraAreaType = 'vpolygon';
        mapdraw.DrawTool = 'polygon';
        // 绘制线样式
        mapdraw.LineStyle = {
            width: 5,
            type: fengmap.FMLineType.FULL,
...
function initMapPoints(polygonCoords, polygonColor) {
    ...
    polygonCoords.forEach(item => {
        ...
        polygon.addTo(floor);
        if (cArea != '') {
            console.log('polygonqqq', polygon);
            if (cArea == 'add') {
                addCameraAreaId(polygon.jn.uuid);
            }
...
let pointCricle = null;
function addCircleLayer(info) {
    if (pointCricle) {
        pointCricle.remove();
    };
    var circle_radius = info.r;
    ...
    pointCricle = new fengmap.FMPolygonMarker(circleOption);
    var floor = fMap.getFloor(+info.floor);

    pointCricle.addTo(floor);
};
