...
function openMap(params, callback, errorCallbak) {
    $.ajax({
        ...
        success: function (res) {
            themeList = res.data;
            openMap2(params, callback, errorCallbak);
        },
...
function openMap2(params, callback, errorCallbak) {

    var hasThemeName = false;
    if (themeList && themeList.length && params.mapId) {
        let item = themeList.find((item) => item.id == params.mapId);
    ...
    var defaultParams = {
        ...
        defaultThemeName: hasThemeName ? hasThemeName : (params.themeName || 'tgymap'),
        defaultViewMode: fengmap.FMViewMode.MODE_2D, //设置地图视图模式为3D
    ...
    fMap.on('loadComplete', function () {
        ...
        var im = new fengmap.FMImageMarker({
            ...
            x: params.x,
            y: params.y,
            // groupID: params.floor
        });
        imgLayer.addMarker(im);
        im.alwaysShow();
        // fMap.addLocationMarker(locationMarker);
        // locationMarker.setPosition({
        //     x:params.x,  //设置定位点的x坐标
        //     y:params.y,  //设置定位点的y坐标
        //     groupID:params.focusFloor  //设置定位点所在楼层
        // });
