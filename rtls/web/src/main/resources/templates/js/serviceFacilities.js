...
function entDele() {
  $.ajax({
    url: url + 'mapBuild/delMapBuild/' + ids + '/wc',
    type: 'get',
    headers: {
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './serviceFacilitiesDetails.html?id=' + id)
  $('#editFrame').show()
}
...
function addCompany() {
  ...
  $('#addFrame').attr('src', './serviceFacilitiesDetails.html')
  $('#addFrame').show()
}
