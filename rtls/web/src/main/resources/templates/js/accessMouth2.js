...
function init() {
    $.ajax({
        url: url + 'crossLevelCorridor/getAllOrConditionalQuery',
        data: where,
        type: 'get',
        beforeSend: function () {
            loading();
...
function entDele() {
    $.ajax({
        url: url + 'crossLevelCorridor/delConditionalById/' + ids,
        type: 'delete',
        beforeSend: function () {
            loading();
        },
...
function detail(id) {
    ...
    $('#editFrame').attr('src', './accessMouthListDetail2.html?id=' + id);
    $('#editFrame').show();
}

function addAccessMouthList() {
    $('#addFrame').attr('style', 'z-index: 99')
    $('#addFrame').attr('src', './accessMouthListDetail2.html')
    $('#addFrame').show()
}
