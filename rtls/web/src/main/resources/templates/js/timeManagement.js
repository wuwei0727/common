...
function init() {
  $.ajax({
    url: url + 'timePeriod/getTimePeriodAdminInfo',
    data: where,
    type: 'get',
    beforeSend: function () {
      loading();
    ...
    success: function (res) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          } else if (nameTar == 'time') {
            html += '<td>' + getJtad(listTar['dayOfWeek']) + ' ' + listTar['startTime'] + ' ~ ' + listTar['endTime'] + '</td>';
          } else {
...
function entDele() {
  $.ajax({
    url: url + 'timePeriod/delTimePeriodAdminInfoById/' + ids,
    type: 'delete',
    beforeSend: function () {
      loading();
    },
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './timeManagementDetail.html?id=' + id)
  $('#editFrame').show()
}
...
function addCompany() {
  ...
  $('#addFrame').attr('src', './timeManagementDetail.html')
  $('#addFrame').show()
}
