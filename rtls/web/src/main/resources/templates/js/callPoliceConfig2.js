    ...
    success: function (res) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          value = target[name];
          if (name === 'line') {
            html += resTabCheck(target, lineNum + i, 'id', 'configName');
          } else if (name === 'operating') {
            var editBtn = document.getElementById("editBtn");
            var deleteBtn = document.getElementById("deleteBtn");
            var editTxt = editBtn ? '<span class="tabOper" onclick="detail(\'' + target.id + '\')">编辑</span>' : '';
            var delteTxt = deleteBtn ? '<span class="tabOper deleOpa" onclick="showDele(\'' + target.id + '\',\'' + target.configName + '\')">删除</span>' : '';
            html = html + '<td>' + editTxt + delteTxt + '</td>';
          } else if (name === 'holiday') {
            if (value) {
              html += '<td class="stateHas">是</td>';
            } else {
              html += '<td class="stateNot">否</td>';
            }
          } else if (name === 'status') {
            if (value) {
              html += '<td class="stateHas">是</td>';
          ...
          } else if (name === 't2TimePeriods1') {
            html += `
                <td>
                  <div class="cheweitd2" style="max-width:200px">${convertNull(value)}</div>
                </td>
            `;
          } else if (name === 't3TimePeriods1') {
            html += `
                <td>
                  <div class="cheweitd2" style="max-width:200px">${convertNull(value)}</div>
                </td>
...
function entDele() {
  $.ajax({
    url: url + 'pac/deleteParkingAlertConfigById/' + ids,
    type: 'DELETE',
    beforeSend: function () {
      loading();
    },
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './cpConfigDetails2.html?id=' + id)
  $('#editFrame').show()
};
...
function add() {
  ...
  $('#addFrame').attr('src', './cpConfigDetails2.html')
  $('#addFrame').show()
};
