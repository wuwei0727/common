		...
		success: function (res) {
			...
			for (var i = 0; i < len; i++) {
				...
				for (var j = 0; j < allName.length; j++) {
					...
					if (nameTar === 'line') {
						html += resTabCheck(listTar, lineNum + i, "id", "callCode");
					} else if (nameTar === 'operating') {
						var editBtn = document.getElementById("editBtn");
						var deleteBtn = document.getElementById("deleteBtn");
						var editTxt = editBtn ? '<span class="tabOper" onclick="detail(\'' + listTar.id + '\')">编辑</span>' : '';
						var delteTxt = deleteBtn ? '<span class="tabOper deleOpa" onclick="showDele(\'' + listTar.id + '\',\'' + listTar.callCode + '\')">删除</span>' : '';
...
function entDele() {
	$.ajax({
		url: url + 'flc/delFloorLockConfigInfo/' + ids,
		beforeSend: function () {
			loading();
		},
		complete: function () {
...
function detail(id) {
	...
	$('#editFrame').attr('src', './floorLockConfigDetails.html?id=' + id)
	$('#editFrame').show()
}

function addFloorLock() {
	$('#addFrame').attr('style', 'z-index: 99')
	$('#addFrame').attr('src', './floorLockConfigDetails.html')
	$('#addFrame').show()
}
