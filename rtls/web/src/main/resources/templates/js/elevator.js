        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          if (nameTar == 'line') {
            ...
            if (nameTar === 'objectType') {
              ...
              if (value == '200005') {
                html += '步行梯';
              } else if (value == '340818') {
                html += '手扶电梯';
              } else if (value == '340855') {
                html += '电梯前室';
              } else if (value == '200004') {
                html += '直升电梯';
              }
              html += '</td>';
            } else {
...
function entDele() {
  ...
  let placeIds = []
  idArr.forEach(item => {
    let listItem = list.filter((list) => {
      return list.id == item
    });
    if (listItem[0].place) {
      placeIds.push(listItem[0].place)
    }
  });

  console.log('placeIds', placeIds.join(','));

  // return false;

  $.ajax({
    url: url + 'peb/delParkingElevatorBinding',
    type: 'post',
    headers: {
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './elevatorDetails.html?id=' + id)
  $('#editFrame').show()
}
...
function addCompany() {
  ...
  $('#addFrame').attr('src', './elevatorDetails.html')
  $('#addFrame').show()
}
