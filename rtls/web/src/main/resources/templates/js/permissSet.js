...
function init(){
    var path;
    path = url + 'company/getPermissionsCompanyById/' + sendId;
    $.ajax({
        url:path,
        type:'post',
        beforeSend:function(){
        ...
        success: function(res) {
            ...
            if(res.code == 200){
                ...
                var info = data.list;
                //设置标题
                $('#topTitle').html('当前权限组：' + info.cname);
...
function save(){
    ...
    var Url;
    var sendData = null;
    Url = url + 'company/companyPermission';
    sendData = {
        cid:sendId,
    }
    sendData.permission_ids = arr.toString();
    sendData.project_ids = proArr.toString();
    $.ajax({
        url: Url,
        type:'post',
        data:sendData,
        ...
        success: function(res) {
            ...
            if(res.code == 200){
                tips(res.message + '，三秒后自动刷新',function(){
                    parent.reloadPage();
                },3000);
            }else{
                tips(res.message);
