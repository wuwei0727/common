...
function init() {
  $.ajax({
    url: url + 'smsQuota/getSmsQuotaOrCondition',
    data: where,
    type: 'get',
    beforeSend: function () {
      loading();
      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          if (nameTar == 'line') {
            ...
            var rechargeTxt = rechargeBtn ? '<span class="tabOper" onclick="recharge(\'' + listTar.id + '\')">充值</span>' : '';
            var editTxt = editBtn ? '<span class="tabOper deleOpa" onclick="detail(\'' + listTar.id + '\')">编辑</span>' : '';
            var delteTxt = deleteBtn ? '<span class="tabOper deleOpa" onclick="showDele(\'' + listTar.id + '\',\'' + listTar.mapName + '\')">删除</span>' : '';
            html = html + '<td>' + rechargeTxt + editTxt + delteTxt + '</td>';
          }
          else {
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './textmessageDetails.html?id=' + id)
  $('#editFrame').show()
}
...
function addCompany() {
  ...
  $('#addFrame').attr('src', './textmessageDetails.html')
  $('#addFrame').show()
}
...
function recharge(id) {
  $('#editFrame').attr('style', 'z-index: 99')
  $('#editFrame').attr('src', './textmessageRecharge.html?id=' + id)
  $('#editFrame').show()
}
