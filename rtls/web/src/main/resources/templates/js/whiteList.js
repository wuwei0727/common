...
function init() {
  $.ajax({
    url: url + 'whitelist/getAllOrFilteredWhitelist',
    data: where,
    type: 'get',
    beforeSend: function () {
      loading();
...
function entDele() {
  $.ajax({
    url: url + 'whitelist/deleteWhitelistById/' + ids,
    type: 'DELETE',
    beforeSend: function () {
      loading();
    },
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './whiteListDetail.html?id=' + id)
  $('#editFrame').show()
};
...
function add() {
  ...
  $('#addFrame').attr('src', './whiteListDetail.html')
  $('#addFrame').show()
};
