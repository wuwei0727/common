      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          if (nameTar === 'line') {
            ...
            var editTxt = editBtn ? '<span class="tabOper deleOpa" onclick="edit(\'' + listTar.id + '\')">编辑</span>' : '';
            var delteTxt = deleteBtn ? '<span class="tabOper deleOpa" onclick="showDele(\'' + listTar.id + '\',\'' + listTar.name + '\')">删除</span>' : '';

            var detailTxt = detailBtn ? '<span class="tabOper" onclick="detail(\'' + listTar.id + '\')">详情</span>' : '';
            var qrcodeTxt = qrcodeBtn ? '<span class="tabOper deleOpa" onclick="QRcode(\'' + listTar.id + '\')">二维码</span>' : '';

            html = html + '<td>' + detailTxt + editTxt + delteTxt + qrcodeTxt + '</td>';
...
function entDele() {
  $.ajax({
    url: url + 'promoterInfo/delPromoterInfo/' + ids,
    beforeSend: function () {
      loading();
    },
    complete: function () {
...
// 详情
function detail(id) {
  $('#detailFrame').attr('style', 'z-index: 99')
  $('#detailFrame').attr('src', './promotionInfo.html?id=' + id)
  $('#detailFrame').show()
};
...
function edit(id) {
  ...
  $('#editFrame').attr('src', './promotionDetail.html?id=' + id)
  $('#editFrame').show()
};
...
function add() {
  ...
  $('#addFrame').attr('src', './promotionDetail.html')
  $('#addFrame').show()
};
...
function QRcode(id) {
  $('#addFrame').attr('style', 'z-index: 99')
  $('#addFrame').attr('src', './promotionQRcode.html?id=' + id)
  $('#addFrame').show()
};
