...
function init() {
    $.ajax({
        url: url + 'crossLevelCorridor/getConditionalById/' + Id,
        beforeSend: function () {
            loading();
        },
        complete: function () {
...
function save() {
    ...
    sendData['type'] = 1; // 先固定传1
    var path = url;
    var type = 'post'
    if (Id) {
        path += 'crossLevelCorridor/editCrossLevelCorridor';
        type = 'put'
        sendData.id = Id;
    } else {
        path += 'crossLevelCorridor/addCrossLevelCorridor';
