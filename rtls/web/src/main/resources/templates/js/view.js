...
$(function () {
    ...
    // 设置根元素的字体大小为屏幕宽度除以设计稿的宽度乘以基准字体大小（一般为16px）
    document.documentElement.style.fontSize = screenWidth / 1920 * 16 + 'px';
    HTMLFONTSIZE = screenWidth / 1920 * 16
    ...
    mapId = params.mapId;
    /* 当链接带有user时-显示查询用户点位查询框 */
    let showUser = params.user;
    if (showUser) {
        $(".searchUserPointMain").css('display', 'block')
    }
...
function getTime() {
    ...
    maxDateTime = curTime.getFullYear() + '-' + zero(curTime.getMonth() + 1) + '-' + zero(curTime.getDate());
    settime = setInterval(getTime, 1000);
}
function zero(str) {
...
function initPie2(dom, param, total) {
    ...
    if (param.data1) {
        ...
        lowPower = param.data[2] || '';
        param.data = [param.data[0], param.data[1]];
    ...
    let v = 4 / 16 * HTMLFONTSIZE;
    let ig = 7 / 16 * HTMLFONTSIZE;
    ...
    if (lowPower) {
        let html = `<div class="newlowPower">
                <div class="lowPowerDian"></div>
                <div class="lowPowerTitle">${lowPower.name}&nbsp;</div>
                <div class="lowPowerNum">${lowPower.value}&nbsp;</div>
            </div>`
        // <div class="lowPowerNum1">(${((lowPower.value / total) * 100).toFixed(2)}%)</div>
        let dd = "#" + dom
        $(dd).append(html)
    }
};
...
function initBar3(dom, params, istodvn) {
    var findCarBar = document.getElementById(dom);
    var bar = echarts.init(findCarBar);
    var option = {
        series: [{
...
function deviceData(data, type) {
    ...
    // var target = contrastReturn(deviceChars, 'type', type);
    // if (target) {
    //     target.offLine = data.offLine;
    //     target.onLine = data.onLine;
    //     target.subLowPower = data.subLowPower;
    // } else {
    //     deviceChars.push({
    //         type: type,
    //         title: title,
    ...
    //             { value: data.subLowPower, name: '低电量' },
    //         ],
    //     });
    //     devicePie.resize();
    // }
    ...
    if (type == 12) {
        ...
        // 信标统计
        let subCount = data.total;
        initPie2('xinbiaoPie', {
            color: ['#4AADFF', '#0667E8', '#7D55FF', '#5A28FE', '#F34040', '#1CED00'],
            data: [
                { value: data.onLine, name: '在线' },
                { value: data.offLine, name: '离线' },
                { value: data.subLowPower || 0, name: '低电量' }
            ],
        }, subCount);

        // 道钉统计
        let count = data.roadSpikeCount;
        if (!count) {
            $("#daoding").hide();
            return
        }
        initPie2('xinbiaoPie1', {
            color: ['#FF7800', '#FFAD2F', '#00BAFF', '#00EED5', '#F34040', '#1CED00'],
            data: [
                { value: data.rsOnLine || 0, name: '在线' },
                { value: data.rsOffLine || 0, name: '离线' },
                { value: data.rsLowPower || 0, name: '低电量' }
            ],
        }, count);
    };
    if (type == 13) {
        // 网关统计
        let gatewayCount = data.total;

        initPie2('wangguanPie', {
    ...
    if (type == 14) {
        ...
        initPie2('jiancePie', {
            ...
            data: [
                { value: data.onLine, name: '在线' },
                { value: data.offLine, name: '离线' },
                { value: data.detectorLowPower || 0, name: '低电量' },
            ],
        }, detectorCount);
    };
};
...
function drawMarker(list, type) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (markerUrl == 'detector') {
            let arr = list.filter((fitem) => {
                return fitem.place == target.place
            });
            if (arr.length >= 2) {
                arr.forEach((aitem, aindex) => {
                    if (aitem.id == target.id) {
                        if (aindex % 2 == 0) {
                            x -= 1 + (aindex * 0.2)
                        }
                    }
...
function createUserNaviObj(id, start, end) {
    ...
    console.log('520', start);
    console.log('520', nowleve);
...
function updateEnter(allEnterData, data) {
    ...
    if (treatUpdateData.length) {
        fordeletedata(data);
    } else {
        treatUpdateData = JSON.parse(JSON.stringify(allEnterData));
    }
};

function fordeletedata(data) {
    var result = JSON.parse(JSON.stringify(treatUpdateData));
    let hadIndex = result.findIndex((item) => {
        return item.carbit == data.carbit
    });
    if (hadIndex == -1) {
        treatUpdateData.unshift(data)
        return
    } else {
        treatUpdateData.splice(hadIndex, 1);
        fordeletedata(data)
    }
};
...
function setTimeoutUpdateHtml() {
    if (setIntervalTime) return;
    setIntervalTime = setInterval(() => {
        let treatUpdateDataLength = treatUpdateData.length;
        console.log('treatUpdateDataLength', treatUpdateDataLength);
        if (treatUpdateDataLength >= 40) {
            let data = JSON.parse(JSON.stringify(treatUpdateData.slice(0, 40)));
            allEnter = data;
            initEnter(allEnter);
        } else if (treatUpdateDataLength > 0) {
            let data = JSON.parse(JSON.stringify(treatUpdateData.slice(0, treatUpdateDataLength)));
            allEnter.unshift(...data);
            allEnter.splice(40)
            initEnter(allEnter);
        };
        treatUpdateData = [];
...
function initPopularLocations() {
    let copyTD = JSON.parse(JSON.stringify(timeData));
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.data.length) {
                ...
                $("#parkingSpaceUtilizationRate").html('')
            }
        },
        error: function (err) {
            console.log(err);
...
function chooseTimeSlot(type, numData) {
    ...
    delete timeData.start;
    delete timeData.end;

    $("#indate").addClass('disableColor');
    $(".selectName").removeClass('disableColor');
    initT1('');

    timeData.time = numData;
...
function getPlaceNavigationUseRate() {
    $.ajax({
        ...
        success: function (res) {
            ...
            if (!res.data.length) {
                $("#carParkNaviga").html('0%')
                $("#carParkNaviga").attr('title', '0%')
                $("#carParkNavigaH").css('height', '0%')
                return
            };
            let placeNavigationTotal = 0;
            res.data.forEach((item) => {
            ...
            placeNavigationTotal = numbersToPercentage(placeNavigationTotal);
...
function getPlatformPlaceUtilizationRate() {
    $.ajax({
        ...
        data: timeData,
        success: function (res) {
            console.log('车位利用率', res);
            if (res.code != 200) {
                return;
            };

            let mapPlatformUtilizationRate = 0;
            res.data.forEach((item) => {
                if (item.map != mapId) return;
                mapPlatformUtilizationRate += +item.mapPlatformUtilizationRate
            });

            mapPlatformUtilizationRate = numbersToPercentage(mapPlatformUtilizationRate);

            $("#carParkUsage").html(mapPlatformUtilizationRate + '%');
            $("#carParkUsage").attr('title', mapPlatformUtilizationRate + '%');
            $("#carParkUsageH").css('height', (mapPlatformUtilizationRate >= 100 ? 100 : mapPlatformUtilizationRate) + '%');
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            var option = {
                tooltip: {
                    trigger: 'item',
                    extraCssText: 'padding: 8px 10px 4px; line-height: 1.5; font-size: .875rem;text-align: center;',
                    position: "top",
                    formatter: function (params) {
...
function getPlaceldleTotalDuration() {
    $.ajax({
        ...
        success: function (res) {
            ...
            res.data.forEach((item) => {
                xArr.push(item.placeName || item.name);
                yArr.push(+item.totalVacantDuration)
            });
...
// 总活跃用户数
function getActiveUserNumber2() {
    $.ajax({
        url: url + 'view/getActiveUserNumber2',
        data: timeData,
        success: function (res) {
            console.log('总活跃用户数', res);
            if (res.code != 200) {
                return;
            };
            // if (!res.data.length) return;
...
function getDeviceCount() {
    $.ajax({
        ...
        success: function (res) {
            ...
            let freeSpaceD = ((data.carBitIdle / total) * 100).toFixed(2);
...
function exprotExcle() {
    let api = '';
    if (timeData.time) {
        api = url + 'variable_operational_data/exportMultiSheet/park?map=' + mapId + '&time=' + timeData.time + '&mapName=' + mapName
    } else {
        api = url + 'variable_operational_data/exportMultiSheet/park?map=' + mapId + '&start=' + timeData.start + '&end=' + timeData.end + '&mapName=' + mapName
    }
...
function getUserPoint() {
    let map = mapId;
    let inputV = $("#Uuserid")[0].value;
    let timeV = $("#Utime")[0].value;
    ...
    $.ajax({
        url: url + 'park/queryUserPositions',
        data: {
            userId: inputV,
            mapId: map,
            startTime: timeV.split('至')[0],
            endTime: timeV.split('至')[1],
        },
        beforeSend: function () {
            loading();
        },
        ...
        success: function (res) {
            ...
            let data = res.data;
            if (data && data.length) {
                addUserPointToMap(data)
                $("#showSearchTip").css('display', 'none')
                $("#showSearchClear").css('display', 'block')
            } else {
                $("#showSearchTip").css('display', 'block')
                $("#showSearchClear").css('display', 'none')
                addUserPointToMap([])
            }
        },
        error: function (jqXHR) {
...
let userMarkerList = [];
function addUserPointToMap(data) {
    if (userMarkerList && userMarkerList.length) {
        userMarkerList.forEach((item) => {
            item.remove()
        });
        userMarkerList = []
    };
    ...
    if (data && data.length) {
        let centerItem = data[0];
        let nowleve = fMap.getLevel();
        if (centerItem.floor != nowleve) {
            fMap.setLevel({
                level: +centerItem.floor,
                animate: false,
            })
        };
        fMap.setCenter({
        ...
        data.forEach((item) => {
            ...
            let imgMarker = new fengmap.FMImageMarker({
                x: +item.x,
                y: +item.y,
                url: '../image/fMap/user-location.png',
                size: 24,
                height: 2,
                collision: false
            });
...
function initT1(num) {
    ...
    jeDate('#indate', {
        ...
        donefun: function (obj) {
            console.log(obj.val);
            $("#indate").removeClass('disableColor');
            $(".selectName").addClass('disableColor');

            delete timeData.time;
            timeData['start'] = obj.val.split('至')[0];
            timeData['end'] = obj.val.split('至')[1];
