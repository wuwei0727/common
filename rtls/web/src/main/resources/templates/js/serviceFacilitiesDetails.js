...
function init() {
  $.ajax({
    url: url + 'mapBuild/getMapBuild2ById/' + ID + '/wc',
    beforeSend: function () {
      loading();
    },
    complete: function () {
...
function save() {
  ...
  var path = url;
  if (ID) {
    path += 'mapBuild/updateMapBuild';
    sendData.id = +ID;
  } else {
    path += 'mapBuild/addMapBuild';
  ...
  if (sendData.objectType == '100004') {
    /* 男洗手间 */
    sendData.iconType = '100004'
  } else if (sendData.objectType == '100005') {
    /* 女洗手间 */
    sendData.iconType = '100005'
  } else if (sendData.objectType == '200010') {
    /* 安全出口 */
    sendData.iconType = '110002'
  }
