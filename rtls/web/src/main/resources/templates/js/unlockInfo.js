...
function loadSeleData() {
  ...
  loadFun('park/getCompany', { pageIndex: 0, pageSize: -1 }, '#companySelect');
}

//初始化列表
function init() {
  $.ajax({
    url: url + 'placeUnlockRecords/getPlaceUnlockRecords',
    data: where,
    type: 'get',
    beforeSend: function () {
      loading();
      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          if (nameTar == 'line') {
            ...
            var delteTxt = deleteBtn ? '<span class="tabOper" onclick="showDele(\'' + listTar.id + '\',\'' + listTar.mapName + '\')">删除</span>' : '';
            html = html + '<td>' + delteTxt + '</td>';
          } else if (nameTar == 'parkingStatus') {
            if (listTar[nameTar] == '1') {
              html += '<td><span class="stateNot">' + '未停放' + '</span></td>';
            } else {
              html += '<td><span class="stateHas">' + '停放' + '</span></td>';
            }
          } else {
...
function entDele() {
  $.ajax({
    url: url + 'placeUnlockRecords/delPlaceUnlockRecordsById/' + ids,
    type: 'delete',
    beforeSend: function () {
      loading();
    },
