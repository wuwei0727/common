...
function processData(res) {
    ...
    var data = item.data;
    if (item.type === 20) {
        // initNumBorder(data[0].userCount, "userTotal");
    } else if (item.type === 24) {
        // initNumBorder(data[0].useFrequency, "visits");
        ...
        initPie('jiancePie', {
            ...
            data: [
                { value: data[0].detectorOnLine, name: '在线' },
                { value: data[0].detectorOffLine, name: '离线' },
                { value: data[0].detectorLowPower || 0, name: '低电量' },
            ],
            // data2: [
            //     { value: data[0].detectorLowPower || 0, name: '低电量' },
            //     { value: data[0].detectorOnLine + data[0].detectorOffLine || 0, name: '电量正常' }
            // ]
        }, data[0].detectorCount);
        initPie('xinbiaoPie', {
            color: ['#4AADFF', '#0667E8', '#7D55FF', '#5A28FE', '#F34040', '#1CED00'],
            data: [
                { value: data[0].subOnLine, name: '在线' },
                { value: data[0].subOffLine, name: '离线' },
                { value: data[0].subLowPower || 0, name: '低电量' }
            ],
            // data2: [
            //     { value: data[0].subLowPower || 0, name: '低电量' },
            //     { value: data[0].subOnLine + data[0].subOffLine || 0, name: '电量正常' }
            // ]
        }, data[0].subCount);
        initPie('wangguanPie', {
            color: ['#8C69FF', '#5A28FE', '#88FBFF', '#3AE6FF'],
            data: [
                { value: data[0].gatewayOnLine, name: '在线' },
...
function initZhe(userTotalNumByMonth) {
    ...
    if (userTotalNumByMonth) {
        userTotalNumByMonth.forEach(item => {
            ...
            // arrNum.push(+item.monthTotalNum);
...
function getPlaceldleTotalDuration() {
    $.ajax({
        ...
        success: function (res) {
            ...
            res.data.forEach((item) => {
                xArr.push(item.mapName);
                yArr.push((item.totalVacantDuration / 60 / 60))
            });
