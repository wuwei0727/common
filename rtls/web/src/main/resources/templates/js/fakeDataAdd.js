            ...
            html += `
                            <div class="cp_list_item" onclick="openNoticeDetail(${item.id})">
                                ...
                                <div class="cp_list_item_bottom">
                                    <div class="cp_bottom_state"><span class="big">设备编号：</span>${(item.num || '未知')}</div>
                                    <div class="cp_bottom_state"><span class="big">报警类型：</span>${getATypeListName(alarmTypeList, item.alarmType)}</div>
...
function openNoticeDetail(id) {
  ...
  switchNav({ id: 'deviceManagNav', className: '' }, 'deviceManag', 3)
  slide.style.left = 3 * 110 + 10 + 'px';
  setTimeout(() => {
