...
function init() {
  $.ajax({
    url: url + 'apb/getAllOrFilteredAlarmPersonnelBindings',
    data: where,
    type: 'get',
    beforeSend: function () {
      loading();
    ...
    success: function (res) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < allName.length; j++) {
          ...
          value = target[name];
          if (name === 'line') {
            html += resTabCheck(target, lineNum + i, 'id', 'mapName');
          } else if (name === 'operating') {
            var editBtn = document.getElementById("editBtn");
            var deleteBtn = document.getElementById("deleteBtn");
            var editTxt = editBtn ? '<span class="tabOper" onclick="detail(\'' + target.id + '\')">编辑</span>' : '';
            var delteTxt = deleteBtn ? '<span class="tabOper deleOpa" onclick="showDele(\'' + target.id + '\',\'' + target.mapName + '\')">删除</span>' : '';
            html = html + '<td>' + editTxt + delteTxt + '</td>';
          } else if (name === 'alarmTypeNames') {
            html += '<td class="cheweitd" style="max-width:750px">' + convertNull(value) + '</td>'
          } else {
...
function entDele() {
  $.ajax({
    url: url + 'apb/deleteAlarmPersonnelBindingsById/' + ids,
    type: 'DELETE',
    beforeSend: function () {
      loading();
    },
...
function detail(id) {
  ...
  $('#editFrame').attr('src', './alarmPushDetail.html?id=' + id)
  $('#editFrame').show()
};
...
function add() {
  ...
  $('#addFrame').attr('src', './alarmPushDetail.html')
  $('#addFrame').show()
};
